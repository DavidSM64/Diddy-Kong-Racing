...
void update_smokey(s32 updateRate, f32 updateRateF, Object *obj, Object_Racer *racer, u32 *input, s32 *stickMag, s32 *startTimer) {
    ...
    *startTimer = sp44;
    ...
    if (obj->segment.object.animationID == 2 && racer->groundedWheels == 0 && racer->velocity < -6.5f) {
    ...
    if (obj68->unk10 == -1 || obj->segment.object.animationID == 0) {
        if (obj->segment.object.animationID == 5) {
            ...
            if (obj->segment.object.animationID == 4 && racer->groundedWheels && racer->velocity > -6.0f) {
                obj->segment.object.animationID = 3;
                racer->unkC = (objModel->animations[obj->segment.object.animationID].unk4 * 16) - 17;
            }
        }
    ...
    for (i = 0; i < sp7C; i++) {
        ...
        if (temp_s1->behaviorId == BHV_UNK_6B) {
            ...
            if ((s8) temp_s2->animation.z_rotation == racer->lap + 1 || (s8) temp_s2->animation.z_rotation == 0) {
                ...
                if (sqrtf((diffX * diffX) + (diffY * diffY) + (diffZ * diffZ)) < (f32) ((s8) temp_s2->animation.x_rotation & 0xFF) * 4.0f) {
