...
void func_80050A28(Object *obj, Object_Racer *racer, s32 updateRate, f32 updateRateF) {
    ...
    if (gDialogueCameraAngle == 0) {
        scale = (f32) D_800DCDB0[0][racer->steerVisualRotation & 0x1F] * 0.025f;
    ...
    if (racer->drifting) {
        ...
        if (racer->drifting == 0) {
            racer->steerVisualRotation += racer->y_rotation_vel * updateRate;
    ...
    velocitdiffY = tempVel - velocityS;
    // Calculate the velocity multiplier based on current velocity
    surfaceTraction = (f32) ((miscAsset[1] * velocitdiffY) + (miscAsset[0] * (1.0f - velocitdiffY)));
    ...
    if (racer->brake < 0.9f && racer->vehicleIDPrev < 5 && gRaceStartTimer == 0) {
        if (racer->pitch < 0.0f) {
            velocitdiffY = -racer->pitch;
            velocitdiffY -= 0.3f;
            if (velocitdiffY < 0.0f) {
                velocitdiffY = 0.0f;
            }
            if (velocitdiffY > 0.1f) {
                velocitdiffY = 0.1f;
            }
            velocitdiffY *= 35.0f;
        } else {
...
void play_random_character_voice(Object *obj, s32 soundID, s32 range, s32 flags) {
    ...
    if (tempRacer->unk108 == 0 && (!(flags & 0x80) || gCurrentPlayerIndex != PLAYER_COMPUTER)) {
        ...
        if (tempRacer->soundMask == NULL && (flags != 3 || get_random_number_from_range(0, 1))) {
            ...
            func_80009558(soundIndex, obj->segment.trans.x_position, obj->segment.trans.y_position, obj->segment.trans.z_position, 4, &tempRacer->soundMask);
...
void update_player_camera(Object *obj, Object_Racer *racer, f32 updateRateF) {
    ...
    gCameraObject->y_velocity = (((func_8003ACAC() + 48.5f) - gCameraObject->trans.y_position) * dialogueAngle);
...
void update_camera_car(f32 updateRate, Object *obj, Object_Racer *racer) {
    ...
    if ((racer->drift_direction && racer->brake > 0.0f)|| gDialogueCameraAngle) {
        newAngle = 0;
    }
...
void racer_enter_door(Object_Racer* racer, s32 updateRate) {
    ...
    gCameraObject->trans.x_position += (temp_a3->unk0 * updateRateF) * 1.5f;
    gCameraObject->trans.z_position += (temp_a3->unk8 * updateRateF) * 1.5f;
