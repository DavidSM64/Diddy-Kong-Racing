...
void AssetTable::_parse_fixed_table() {
    uint32_t offset = _romOffset;
    int numEntries = _rom->get_s32(offset);
    offset += 4;
    for(int i = 0; i < numEntries; i++) {
        int32_t entryValue = _rom->get_s32(offset);
        int32_t entrySize = _rom->get_s32(offset + 4) - entryValue;
...
void AssetTable::_parse_variable_table() {
    ...
    while(true) {
        int32_t entryValue = (_rom->get_s32(offset));
        int32_t nextEntryValue = (_rom->get_s32(offset + 4));
...
void AssetTable::_parse_ttghost_table() {
    ...
    while(true) {
        ...
        int32_t entryValue = (_rom->get_s32(offset + 4));
        int32_t nextEntryValue = (_rom->get_s32(offset + 12));
        if(entryValue == -1 || nextEntryValue == -1) {
            return;
        }
        int32_t entrySize = nextEntryValue - entryValue;
...
void AssetTable::_parse_audio_table() {
    ...
    int32_t firstEntrySize = _rom->get_s32(offset);
