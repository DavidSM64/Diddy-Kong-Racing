...
void BuildModelBlock::write_batches(DkrBatch*& outBatches, DkrVertex*& outVertices, DkrTriangle*& outTriangles) {
    size_t vertexCount = 0;
    size_t triCount = 0;
    for(auto &batch : _batches) {
        batch.write_batch(outBatches, outVertices, outTriangles);
