...
AssetEnums::AssetEnums() {
    _outFilepath = GlobalSettings::get_decomp_path("include_subpath", "include/") / "asset_enums.h";
...
void AssetEnums::_write_asset_section_enums() {
    ...
    for(std::string &sectionId : mainOrder) {
        ...
        JsonFile &sectionJson = AssetsHelper::get_asset_section_json(sectionId);
...
void AssetEnums::_write_single_asset_section_enums(const std::string &sectionId, const JsonFile &sectionJson) {
    ...
    fs::path jsonPath = GlobalSettings::get_decomp_path_to_output_assets();
    if(!subFolder.empty()) {
        jsonPath /= subFolder;
    }
    ...
    auto tryGetAssetJson = JsonHelper::get_file(jsonPath);
    DebugHelper::assert(tryGetAssetJson.has_value(),
        "(AssetEnums::_write_single_asset_section_enums) Could not load fonts json!");
    JsonFile &assetJson = tryGetAssetJson.value();
...
void AssetEnums::_write_asset_section_enums(const std::string &sectionId, const JsonFile &sectionJson) {
    std::string sectionEnumName = StringHelper::upper_snake_case_to_pascal_case(sectionId) + "Enum";
    WriteableCEnum sectionEnum(sectionEnumName);
...
void AssetEnums::_write_deferred_asset_section_enums(const std::string &sectionId, const JsonFile &mainJson, const std::string &deferInfoPtr) {
    std::string sectionEnumName = StringHelper::upper_snake_case_to_pascal_case(sectionId) + "Enum";
    WriteableCEnum sectionEnum(sectionEnumName);
